...
public class DataHero {
    ...
    public static ArrayList<HeroesModel> getListHero(){
        HeroesModel heroesModel = null;
        ArrayList<HeroesModel> list = new ArrayList<>();
        for (int i = 0; i<heroNames.length; i++){
            heroesModel = new HeroesModel();
            heroesModel.setHeroName(heroNames[i]);
            heroesModel.setHeroDetail(heroDetails[i]);
            heroesModel.setHeroPict(heroesImages[i]);
